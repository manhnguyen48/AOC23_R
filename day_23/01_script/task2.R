...
edge_contract <- function(mtx, poi) {
  ...
  for (pt in poi) {
    ...
    while (q$size() > 0) {
      ...
      #This is the only difference from part 1 as we ignore the slopes
      for (dxy in list(c(-1, 0), c(1, 0), c(0, -1), c(0, 1))) {
        nx <- x + dxy[1]
        ny <- y + dxy[2]
        if (nx >= 1 &&
            nx <= nrow(mtx) &&
...
longest_path(parsed_test) == 154
#Brute force might take a while
#Answer Part 2: 6398
